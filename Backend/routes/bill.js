        ...
        if (!err) {
            ejs.renderFile(path.join(__dirname, '', "report.ejs"), {
                ...
                if (err) {
                    ...
                    // Fixed: removed extra space in filename
            ...
            if (err) {
                ...
                pdf.create(results).toFile('./generated_pdf/' + orderDetails.uuid + ".pdf", function (err, data) {
                    if (err) {
                        ...
                        // Fixed: send the file after creating it
                        res.contentType("application/pdf");
                        fs.createReadStream('./generated_pdf/' + orderDetails.uuid + ".pdf").pipe(res);
                    }
                })
            }
        })
